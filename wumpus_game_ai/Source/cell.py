...
class Cell:
    def __init__(self, matrix_pos, map_size, objects_str):
        self.matrix_pos = matrix_pos                                            # map ban đầu: (0, 0) (0, 1) ... (9, 9)   (TL -> BR)
        self.map_pos = matrix_pos[1] + 1, map_size - matrix_pos[0]              # đổi map: (1, 1) (1, 2) ... (10, 10) (BL -> TR)
        self.index_pos = map_size * (self.map_pos[1] - 1) + self.map_pos[0]     # set index từng ô: 1 2 3 ... 99 100           (BL -> TR)
        self.map_size = map_size # 10x10

        """ Khởi tạo các thuộc tính của cell"""
        self.explored = False        
        self.percept = [False, False, False, False, False]  # [-G, -P, -W, -B, -S]
        self.initialize_percepts(objects_str)

        self.parent = None
        self.child_list = []

    """ Khởi tạo các percept của cell """
    def initialize_percepts(self, objects_str):
    ...
    def kill_wumpus(self, cell_matrix, kb):
        ...
        for stench_cell in adj_cell_list_of_wumpus_cell:
            ...
            """ Kiểm tra xem có còn ô nào có mùi hôi không """
            ...
            """ Nếu không còn ô nào có mùi hôi thì xóa mùi hôi của ô hiện tại """
            if del_stench_flag:
    ...
    """ Trả về một số cho các đối tượng của cell
        Ví dụ:
        P: 10001
        W: 10002
        B: 10003
        S: 10004
        '-': đảo dấu (biểu thị ô trống)
    """
